...
const list = ref([])
const columns = ['Id', 'Writer Name', 'About']
const baseApi = process.env.VUE_APP_BASE_API 

function refresh() {
    axios.get(baseApi+'writer/list').then((response) => {
       list.value = response.data.data
    })
}

onMounted(async () => {
    await axios.get(baseApi+'writer/list').then((response) => {
       list.value = response.data.data
    })
})
...
<template>
    <div class="">
        ...
        <Card>
            <Table :list="list" :columns="columns" :delete-api="'writer/delete/'"  @delete="refresh" />
        </Card>
